...
- **Catalogs live in `core/api/catalogs/`** (types, `api.ts`, `queries.ts`), keyed `['catalogs', name, ...params]` — e.g. `['catalogs', 'districts', departmentId, provinceId]`. They override `staleTime` with `Infinity`: reference data does not change within a session. A dependent select passes `enabled` at the call site, not in the query options.
